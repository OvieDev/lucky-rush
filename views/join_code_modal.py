        ...
        if self.code.value in sessions:
            if sessions[self.code.value].guild == interaction.guild:
                ...
                await interaction.response.send_message(
                    content=f"You're in the wrong server! This code is for: {sessions[self.code.value].guild.name}",
                    ephemeral=True)
        else:
            await interaction.response.send_message(content="Cannot find session :(", ephemeral=True)
